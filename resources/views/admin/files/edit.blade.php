                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header py-3">
                                <h6 class="m-0 font-weight-bold float-left">Edit File Uploaded
                                </h6>
                                <a class="btn btn-primary btn-sm float-right"href="{{ route('files.index') }}"><i
                            ...
                            <div class="card-body">
                                ...
                                <form action="{{ route('files.update', $file->id) }}" method="POST"
                                    ...
                                    <div class="form-group">
                                        <label for="cover_image">Choose New Cover Image (JPEG, PNG, JPG, GIF, SVG, or
                                            PDF):</label>
                                    ...
                                    <table class="table table-bordered" id="categories-table">
                                        ...
                                        <tbody>
                                            ...
                                            <button type="button" class="btn btn-primary btn-sm float-right"
                                                style="margin:5px" id="add-category" data-toggle="tooltip"
                                                    ...
                                                    <td>
                                                        <select class="form-control category-select" disabled>
                                                            @foreach ($categories as $cat)
                                                                <option value="{{ $cat->id }}"
                                                                    {{ $fileCategory->file_category_id == $cat->id ? 'selected' : '' }}>
                                                                    {{ $cat->name }}</option>
                                                            @endforeach
                                                        </select>
                                                    </td>
                                                    <td><input type="text" class="form-control category-amount"
                                                            value="{{ $fileCategory->amount }}"></td>
                                                    <td>
                                                        <input type="file" class="form-control" disabled>
                                                        @if ($fileCategory->file_path)
                                                            <a href="{{ asset('storage/' . $fileCategory->file_path) }}"
                                                                target="_blank">Current File</a>
                                                        @endif
                                                    </td>
                                                    <td>
                                                        <a class="btn btn-primary edit-btn">Edit</a>
                                                        <a href="{{ route('file.category.upload.delete', $fileCategory->id) }}"
                                                            class="btn btn-danger btn-sm dltBtn">Delete</a>
                                                    </td>
                                                </tr>
                                            @endforeach
            ...
            <div class="modal fade" id="editModal" tabindex="-1" aria-labelledby="editModalLabel" aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <form action="{{ route('file.category.update') }}" method="POST" enctype="multipart/form-data">
                            @csrf
