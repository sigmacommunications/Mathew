...
class FileUploadController extends Controller
{
    ...
    public function destroy($id)
    {
        ...
        // }
        try {
            $file = FileUpload::with('categories')->findOrFail($id);

            // Delete the cover image from storage
            if ($file->cover_image && Storage::disk('public')->exists($file->cover_image)) {
                Storage::disk('public')->delete($file->cover_image);
            }

            // Delete each category file (stored in the pivot table)
            foreach ($file->categories as $category) {
                $pivotData = $category->pivot;
                if ($pivotData->file_path && Storage::disk('public')->exists($pivotData->file_path)) {
                    Storage::disk('public')->delete($pivotData->file_path);
                }
            }
